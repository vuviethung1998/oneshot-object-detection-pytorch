...
class coco(imdb):
  ...
  def filter(self, seen=1):
    ...
    if seen==1:
      ...
      if len(self.list)==1:
        self.list = [self.coco_class_ind_to_cat_id[cat] for cat in range(1,self.num_classes) if cat%4 != self.list[0]]
      ...
      # Group number to class
      if len(self.list)==2:
        self.list = [self.coco_class_ind_to_cat_id[cat] for cat in range(1,self.num_classes)]
